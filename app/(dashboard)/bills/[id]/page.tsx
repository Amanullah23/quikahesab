...
export default async function BillDetailPage({
  ...
  const customer: any = Array.isArray(bill.customers)
    ? bill.customers[0]
    : bill.customers;
  const pkg: any = Array.isArray(bill.packages)
    ? bill.packages[0]
    : bill.packages;
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <span
          className={`px-3 py-1 rounded-full text-xs font-semibold capitalize shrink-0 ${STATUS_STYLES[bill.status]}`}
        >
          {bill.status}
        </span>
