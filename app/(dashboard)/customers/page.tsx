...
export default async function CustomersPage({
  ...
  let query = supabase
    .from("customers")
    .select("id, customer_number, full_name, whatsapp_number, whatsapp_valid, is_active, packages(name)")
    ...
    <div>
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-text-muted mt-1">{customers?.length ?? 0} customers shown</p>
        </div>
        <div className="flex items-center gap-3">
          <Link
            href="/customers/import"
            className="flex items-center gap-2 border border-border text-text px-5 py-2.5 rounded-full text-sm font-medium hover:bg-bg active:scale-95 transition duration-150"
          >
            <Upload size={15} />
            Import CSV
          </Link>
          <Link
            href="/customers/new"
            className="flex items-center gap-2 bg-forest text-white px-5 py-2.5 rounded-full text-sm font-medium hover:opacity-90 active:scale-95 transition duration-150"
          >
            <Plus size={16} />
            Add Customer
          </Link>
