...
export default async function DashboardHome() {
  ...
  const cards = [
    { label: "Pending", value: counts.pending, badge: "bg-badge-amber-bg text-badge-amber-text", href: "/bills?status=pending" },
    { label: "Partial", value: counts.partial, badge: "bg-badge-amber-bg text-badge-amber-text", href: "/bills?status=partial" },
    { label: "Paid", value: counts.paid, badge: "bg-badge-green-bg text-badge-green-text", href: "/bills?status=paid" },
    { label: "Cancelled", value: counts.cancelled, badge: "bg-badge-red-bg text-badge-red-text", href: "/bills?status=cancelled" },
  ];
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          <Link
            ...
            className="flex items-center gap-2 border border-border text-text px-4 py-2.5 rounded-full text-sm font-medium hover:bg-bg active:scale-95 transition duration-150"
          >
            <Upload size={15} />
            Import CSV
          </Link>
      ...
      <AnimatedGrid className="grid grid-cols-4 gap-4 mb-6">
        {cards.map((c, i) => {
          ...
          return (
            <Link
              ...
              className={`rounded-3xl p-5 transition hover:shadow-md ${
                isHighlighted ? "bg-forest text-white" : "bg-card border border-border text-text"
              }`}
            >
              <div className="flex items-center justify-between mb-5">
                <p className={`text-sm ${isHighlighted ? "text-white/70" : "text-text-muted"}`}>
                  {c.label}
                </p>
                <span
                ...
                >
                  <ArrowUpRight size={15} className={isHighlighted ? "text-white" : "text-text-muted"} />
                </span>
              </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-card border border-border rounded-3xl p-5">
          ...
          <span className="font-display text-4xl font-bold text-text">{todayCount}</span>
          <p className="text-sm text-text-muted mt-3">
        ...
        <div className="bg-card border border-border rounded-3xl p-5 col-span-2">
          <h2 className="text-sm text-text-muted mb-4">
            Overdue Customers <span className="text-text-muted/70">(2+ unpaid bills)</span>
          </h2>
          {overdue && overdue.length > 0 ? (
              ...
              <tbody>
                {overdue.map((o: any) => (
                  <tr key={o.id} className="border-t border-border">
                    <td className="py-2 text-text font-medium">{o.full_name}</td>
            ...
            <p className="text-sm text-text-muted py-4">No overdue customers right now 🎉</p>
          )}
        </div>
      </div>
    </div>
