  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="font-display text-3xl font-bold text-text">Packages</h1>
          <p className="text-text-muted mt-1">{packages?.length ?? 0} packages</p>
        </div>
        <Link
          href="/packages/new"
      ...
      <div className="bg-card border border-border rounded-3xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {packages?.map((p) => (
              <tr key={p.id} className="border-t border-border hover:bg-bg transition">
                <td className="px-5 py-3.5 font-semibold text-forest">
                  <Link href={`/packages/${p.id}/edit`} className="hover:underline">
                    {p.name}
                  </Link>
                </td>
        ...
        {packages?.length === 0 && (
          <p className="text-center text-text-muted py-12 text-sm">No packages yet</p>
        )}
      </div>
    </div>
  );
